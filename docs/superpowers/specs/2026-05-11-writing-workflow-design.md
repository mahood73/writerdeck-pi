...
On exit from WordGrinder, the existing `wd-session` menu appears: `w` reopen · `s` shell · `r` reboot · `p` poweroff. No changes to this loop.
...
- `~/Writing` is the sync root (user-configurable at install time).
...
- **Sync agnosticism:** Remove Syncthing-specific commands from `wd`. Writing root should be a folder the user points any sync tool at.
